...
| `Model`             | `model_id`                  | Первичный ключ, уникальный идентификатор модели                        |
|                     | `version_number`            | Номер версии модели                                                    |
|                     | `fine_tuned`                | Является ли модель точно настроенной под конкретную задачу             |
|                     | `created_at`                | Дата и время создания модели                                           |
|                     | `status`                    | Статус модели (например, активная, устаревшая)                         |
...
| --------------------------- | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| User, User_settings         | PostgreSQL                              | Эти таблицы хранят личную информацию о пользователях и настройках. Их можно хранить в одной базе данных, поскольку эти данные связаны и часто используются вместе. Обращения к ним происходят относительно редко, поэтому можно использовать PostgreSQL.                                                                                                                                 |
| Model                       | PostgreSQL                              | В этой таблице данные изменяются редко, в основном происходит чтение, нагрузку, возникающую при частых обращениях, можно решить кэшированием.                                                                                                                                                                                                                                            |
| Media, Chats, Chat_messages | Cassandra                               | Таблицы, связанные с чатами и сообщениями, нужно вынести в отдельную базу данных, так как они требуют быстрой записи и поиска сообщений. Сообщения могут включать в себя текстовые данные и медиа, что требует высокой производительности при чтении и записи. Cassandra хорошо подходит для горизонтального масштабирования и высокой доступности при работе с чатами и их сообщениями. |
| Model_performance           | ClickHouse                              | Данные о производительности моделей также могут быть изолированы для того, чтобы обеспечить удобное масштабирование и обработку этих данных, так как их может быть много, и они связаны с производительностью системы. Колоночная база данных, такая как ClickHouse, хорошо подойдет для отслеживания временных метрик производительности моделей.                                       |
...
| Таблицв             | Шардирование                                             | Резервирование                                       |
| ------------------- | -------------------------------------------------------- | ---------------------------------------------------- |
| `User`              | По `user_id`                                             | Инкрементальное резервное копирование                |
| `Model`             | Шардирование не требуется                                | Полное резервное копирование + Репликация для чтения |
| `Model_performance` | По диапазону `evaluation_window_start` ~ `calculated_at` | Инкрементальное резервное копирование                |
| `Chats`             | По `user_id` и `last_updated`                            | Инкрементальное резервное копирование + репликация   |
| `Chat_messages`     | По `chat_id`                                             | Инкрементальное резервное копирование + репликация   |
| `Media`             | По `user_id` и `uploaded_at`                             | Инкрементальное резервное копирование + репликация   |
| `Files`             | Зависит от S3                                            | Резервирование на уровне хранилища S3                |
| `Session`           | Шардирование не требуется                                | Резервирование не требуются, данные временные        |
