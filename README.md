...
- Nginx
  - поддержка различных алгоритмов балансировки: Nginx позволяет использовать различные методы распределения трафика
...
- Kubernetes
  - автоматически перезапускает упавшие контейнеры и перераспределяя поды при сбоях узлов (readiness), тем самым поддерживая непрерывность работы приложений. Он также позволяет легко масштабировать приложения (autoscaling).
- Nginx
...
Суммарный RPS = 1026.74

Учитывая, что в среднем установка соединения занимает ~ 4 мс, т.к. будут переиспользоваться защищенные сессии (session ticket), то на обработку SSL в среднем потребуется 1026.74 \* 4 мс ~= 4107 мс ~= 4,1 с
