...
![](img/database_logic_scheme.jpg)

| Название таблицы  | Поля                                                                                                                                          | Описание                                                                                                                                                | Размеры данных |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **User**          | user_id (PK), email, password_hash, account_status, created_at                                                                                | Хранит информацию о пользователе, такую как email, хэшированный пароль, статус аккаунта и дату создания.                                                | -              |
| **User_settings** | setting_id (PK), user_id (FK), theme, language, memory, custom_pre_prompt, custom_personalize_prompt, connected_apps, notification_preference | Содержит настройки пользователя, такие как тема, язык, память, персонализированные подсказки и настройки уведомлений. Связано с таблицей пользователей. | -              |
| **Media**         | media_id (PK), user_id (FK), file_url, uploaded_at                                                                                            | Хранит медиафайлы, загруженные пользователями, включая ссылку на файл и идентификатор пользователя, который его загрузил.                               | -              |
| **Chats**         | chat_id (PK), user_id (FK), title, msg_count, last_updated                                                                                    | Представляет чат-сессии. Каждый чат связан с пользователем и содержит заголовок, количество сообщений и дату последнего обновления.                     | -              |
| **Chat_messages** | msg_id (PK), chat_id (FK), media_id (FK), content, author, last_updated                                                                       | Хранит отдельные сообщения в чате, которые могут включать медиафайлы. Каждое сообщение связано с чатом и может ссылаться на медиафайл.                  | -              |
| **Rating**        | rating_id (PK), msg_id (FK), rating                                                                                                           | Хранит оценку для конкретного сообщения в чате. Каждая оценка связана с сообщением.                                                                     | -              |
| **Session**       | session_id (PK), user_id (FK), sid_hash                                                                                                       | Хранит hash от сессии. Каждая сессия связана с пользователем.                                                                                           | -              |
