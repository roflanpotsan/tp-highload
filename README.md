...
| Открытие страницы чата | 208,4 | 312,6 |
...
| ---- | ---- | ---- | ----|
| Открытие страницы чата | 208,4 * (1,8 КБ + 0.5 МБ) ~ 52,3 МБ/c ~ 418 Мбит/c | 312,6 * (1,8 КБ + 0.5 МБ) ~ 627 Мбит/с | 418 Мбит/c * 86400 c ~ 4,52 ТБ/сут |
...
- Основной домен **chatgpt.com**
  - Предназначен для раздачи front-end части приложения, все остальное взаимодействие с backend сервисами происходит через соответствующие поддомены. 
- CDN **media.chatgpt.com**
  - Предназначен для раздачи и хранения пользовательского медиа-контента, статики.
- Авторизация **auth.chatgpt.com**
  - Предназначен для аутентификации, авторизации и хранения данных пользователя.
- Взаимодействие с моделью **api.chatgpt.com**
  - Предназначен для взаимодействия с моделью, отправки различных запросов ей.
### Обоснования расположения ДЦ
- Сервера OpenAI находятся исключительно на территории США по данным с официального сайта компании [[5](#список-источников)]. Это так же подтверждается при попытке произвести nslookup (из Москвы без VPN).
>
> ![](img/nslookup.png)
>
> ![](img/nsip2.png)
