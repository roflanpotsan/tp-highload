...
[_11. Расчёт ресурсов_](#11-расчёт-ресурсов)
...
**Event-driven architecture** - архитектура, основанная на асинхронной обработке событий, где компоненты системы реагируют на события, публикуемые другими, таким образом если один компонент выйдет из строя, это напрямую не повлияет на другие. События, которые не получилось обработать, помещаются в очередь, для отложенной обработки.
...
Например, при загрузке файлов: если отключится FileProcessor, то данные будут загружаться в хранилище для необработанных файлов и читаться оттуда же.

Например, при инференсе, при невозможности получить доступ к определенной модели, будет попытка получить схожую (или менее производительную модель), при неудаче будет вызван шаблонный генератор ответа, который будет выполнять поиск по embedding'ам для генерации приемлемого ответа, хоть он и не будет на уровне ответа LLM, но пользователь получит ответ.
...
**Event-driven architecture** - архитектура, основанная на асинхронной обработке событий, где компоненты системы реагируют на события, публикуемые другими, таким образом если один компонент выйдет из строя, это напрямую не повлияет на другие. События, которые не получилось обработать, помещаются в очередь, для отложенной отработки.
...
**KubeFlow** - позволяет фиксировать метрики экспериментов и историю обучения, чтобы отслеживать эффективность моделей и их улучшения с течением времени. Это помогает анализировать, как модели вели себя в процессе разработки.
...
**Redis Database (RDB)** - данный метод резервного копирования позволяет создавать периодические снимки данных, обеспечивая высокую производительность и восстановление данных при сбоях.
...
| Inference           | 16 cores         | 64 GB            | 8 GPUs           | 16          |
| ModelStorage        | 8 cores          | 32 GB            | -                | 10 (2 \* 5) |
| DataProcessing      | 16 cores         | 64 GB            | 8 GPUs           | 32          |
| ModelTraining       | 16 cores         | 64 GB            | 8 GPUs           | 50          |

| Название | Хостинг | Конфигурация                                                    | Cores | Count | Покупка  | Аренда        | Окупится через |
| -------- | ------- | --------------------------------------------------------------- | ----- | ----- | -------- | ------------- | -------------- |
| kubenode | own     | 2x6338 / 10x64GB DDR5 RAM / 8xH100 NVL GPU / 4TB NVMe / 100Gb/s | 64    | 25    | 6779300$ | 1440000$/мес. | ~5 мес.        |
