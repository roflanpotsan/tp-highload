...
| ------------------- | --------------------------- | ----------------------------------------------------------------------------------------- |
| `User`              | `user_id`                   | Первичный ключ, уникальный идентификатор пользователя                                     |
|                     | `email`                     | Электронная почта пользователя                                                            |
|                     | `password_hash`             | Хеш пароля                                                                                |
|                     | `account_status`            | Статус аккаунта пользователя (например, активный, неактивный)                             |
|                     | `created_at`                | Дата и время создания пользователя                                                        |
| `User_settings`     | `setting_id`                | Первичный ключ, уникальный идентификатор настроек пользователя                            |
|                     | `user_id`                   | Внешний ключ, ссылающийся на таблицу `User`                                               |
|                     | `theme`                     | Тема интерфейса пользователя (например, светлая, темная)                                  |
|                     | `language`                  | Предпочитаемый язык пользователя                                                          |
|                     | `memory`                    | Включить ли запоминание сообщений пользователя для накопления конекста                    |
|                     | `custom_pre_prompt`         | Пользовательский промпт для персонализации запросов                                       |
|                     | `custom_personalize_prompt` | Пользовательский промпт для персонализации ответов                                        |
|                     | `notification_preference`   | Настройки уведомлений пользователя                                                        |
|                     | `api_key`                   | API-ключ пользователя для интеграций                                                      |
| `Media`             | `media_id`                  | Первичный ключ, уникальный идентификатор медиафайла                                       |
|                     | `user_id`                   | Внешний ключ, ссылающийся на таблицу `User`                                               |
|                     | `file_url`                  | URL-адрес расположения медиафайла                                                         |
|                     | `file_type`                 | Тип медиафайла (например, изображение, документ)                                          |
|                     | `uploaded_at`               | Дата и время загрузки медиафайла                                                          |
| `Chats`             | `chat_id`                   | Первичный ключ, уникальный идентификатор чата                                             |
|                     | `user_id`                   | Внешний ключ, ссылающийся на таблицу `User`                                               |
|                     | `model_id`                  | Внешний ключ, ссылающийся на таблицу `Model`                                              |
|                     | `title`                     | Название чата                                                                             |
|                     | `last_updated`              | Дата и время последнего сообщения в чате                                                  |
| `Session`           | `session_id`                | Первичный ключ, уникальный идентификатор сессии                                           |
|                     | `user_id`                   | Внешний ключ, ссылающийся на таблицу `User`                                               |
|                     | `sid_hash`                  | Хеш идентификатора сессии для безопасности                                                |
|                     | `created_at`                | Дата и время создания сессии                                                              |
|                     | `expires_at`                | Дата и время истечения срока действия сессии                                              |
| `Model_version`     | `model_version_id`          | Первичный ключ, уникальный идентификатор версии модели                                    |
|                     | `parent_model_version_id`   | Идентификатор предыдущей версии модели (если существует)                                  |
|                     | `training_id`               | Идентификатор данных об обучении/(дообучении, если существует родительская модель) модели |
|                     | `file_url`                  | Ссылка на бинарный файл модели                                                            |
|                     | `created_at`                | Дата и время создания модели                                                              |
| `Model_performance` | `performance_id`            | Первичный ключ, уникальный идентификатор записи производительности                        |
|                     | `model_id`                  | Внешний ключ, ссылающийся на таблицу `Model`                                              |
|                     | `metric_type`               | Тип метрики (например, точность, скорость, общий рейтинг)                                 |
|                     | `metric_value`              | Значение метрики                                                                          |
|                     | `calculated_at`             | Дата и время вычисления метрики                                                           |
|                     | `evaluation_window_start`   | Время начала окна оценки                                                                  |
| `Files`             | `file_id`                   | Первичный ключ, уникальный идентификатор файла                                            |
|                     | `file_url`                  | URL-адрес расположения файла                                                              |
|                     | `data`                      | Данные файла в бинарном формате                                                           |
| `Chat_messages`     | `msg_id`                    | Первичный ключ, уникальный идентификатор сообщения                                        |
|                     | `chat_id`                   | Внешний ключ, ссылающийся на таблицу `Chats`                                              |
|                     | `media_id`                  | Внешний ключ, ссылающийся на таблицу `Media`                                              |
|                     | `content`                   | Содержание сообщения                                                                      |
|                     | `author`                    | Автор сообщения                                                                           |
|                     | `last_updated`              | Дата и время последнего обновления сообщения                                              |
|                     | `is_prompt`                 | Является ли сообщение промптом                                                            |
|                     | `is_memo`                   | Является ли сообщение частью накопленного контекста                                       |
|                     | `via_api`                   | Было ли сообщение отправлено через API                                                    |
|                     | `rating`                    | Оценка сообщения (например, для обратной связи)                                           |
|                     | `embedding_vector`          | Векторное представление сообщения                                                         |
|                     | `tokens_used`               | Количество использованных токенов в сообщении                                             |
| `Training`          | `training_id`               | Идентификатор обучения                                                                    |
|                     | `hyper_parameters`          | Гиперпараметры модели                                                                     |
|                     | `eval_score`                | Оценка f1-score, accuracy и других показателей модели                                     |
|                     | `train_start_time`          | Время начала обучения                                                                     |
|                     | `train_end_time`            | Время окончания обучения                                                                  |
| `Training_data`     | `training_data_id`          | Идентификатор обучающего набора данных                                                    |
|                     | `training_id`               | Ссылка на идентификатор обучения                                                          |
|                     | `label_id`                  | Ссылка на идентификатор разметки                                                          |
|                     | `embedding_vectors`         | Векторные представления данных                                                            |
|                     | `created_at`                | Дата и время создания обучающего набора данных                                            |
| `Labels`            | `label_id`                  | Идентификатор метки                                                                       |
|                     | `file_url`                  | URL к размеченному файлу                                                                  |
|                     | `source_text`               | Исходный размеченный текст                                                                |
|                     | `data_type`                 | Тип данных разметки                                                                       |
|                     | `label_type`                | Типразмметки                                                                              |
|                     | `label_status`              | Статус разметки                                                                           |
|                     | `created_at`                | Дата и время создания разметки                                                            |
| `Model_deployment`  | `deployment_model_id`       | Идентификатор развертывания модели                                                        |
|                     | `model_version_id`          | Ссылка на идентификатор версии модели                                                     |
|                     | `status`                    | Статус модели                                                                             |
|                     | `deployed_at`               | Дата и время развертывания модели                                                         |
| `Event`             | `event_id`                  | Идентификатор события                                                                     |
|                     | `user_id`                   | Ссылка на идентификатор пользователя                                                      |
|                     | `chat_id`                   | Ссылка на идентификатор чата                                                              |
|                     | `media_id`                  | Ссылка на идентификатор медиафайла                                                        |
|                     | `model_version_id`          | Ссылка на идентификатор версии модели                                                     |
|                     | `performance_id`            | Ссылка на идентификатор производительности модели                                         |
|                     | `training_data_id`          | Ссылка на идентификатор обучающих данных                                                  |
|                     | `training_id`               | Ссылка на идентификатор обучения                                                          |
|                     | `deployment_model_id`       | Ссылка на идентификатор развертывания модели                                              |
...
| Model_performance, Model_version, Labels, Training_data | ClickHouse                              | ClickHouse — это колоночная СУБД, которая оптимизирована для высокоскоростной обработки аналитических запросов и агрегаций на больших объемах данных. Таблицы, связанные с обучением моделей и анализом их производительности (например, Training_data, Labels, Model_versions, Model_performance), требуют больших объемов данных, которые анализируются для получения статистики и показателей производительности моделей. ClickHouse позволяет обрабатывать такие аналитические запросы намного быстрее, чем другие СУБД. |
...
| **Labels (ClickHouse)**            | **Не требуется** — относительно небольшие данные по меткам.                                                                                       | **Резервирование** на уровне кластера ClickHouse (репликация), резервное копирование аналогично с `Training_data`.                                                                                                 |
...
Параллелизм данных - основная техника распределенного обучения, при которой набор данных распределяется между несколькими GPU, каждый из которых имеет полную копию модели.
Существует два основных подхода к обновлению состояния модели на разных графических процессорах

- Синхронные обновления: Градиенты агрегируются, а параметры модели обновляются одновременно на всех GPU
- Асинхронные обновления: Каждый GPU обновляет свою локальную модель независимо, обмениваясь обновлениями асинхронно.

Асинхронные обновления могут повысить производительность обучения, но могут привести к таким проблемам, как нестабильность, дивергенция и узкие места из-за несогласованности состояний моделей на разных GPU.

**Ring-AllReduce**, децентрализованный асинхронный алгоритм, обеспечивает эффективное решение за счет организации графических процессоров в кольцевую топологию. Это позволяет обеспечить эффективную связь между соседними GPU, устранить необходимость в центральном сервере параметров и синхронизировать обновления параметров для поддержания согласованного состояния модели.

**Ring-AllReduce** состоит из двух основных этапов. Scatter-Reduce агрегирует градиенты, передавая элементы между соседними GPU. AllGather разделяет агрегированные градиенты между всеми GPU.

### Model Parallelism (параллелизм модели)

Параллелизм моделей - это техника машинного обучения, при которой вычислительная нагрузка на нейронную сеть распределяется на несколько устройства или процессоров. В отличие от параллелизма данных, при котором разные партии данных независимо обучают копии модели, параллелизм моделей предполагает разделение одной нейронной сети на множество устройств, каждое из которых отвечает за вычисление части операций модели.

### Распределенные алгоритмы для инференса (предсказаний)

Генерация ответов на основе обученной модели также может быть распределена. Когда запрос от пользователя поступает в систему, задача генерации ответа может быть распределена между несколькими узлами для увеличения скорости обработки. (С помощью pipeline parallelism + дополнительной балансировки запросов)

#### Pipeline Parallelism

Один из примитивных видов параллелизма для глубокого обучения.Он позволяет разделить процесс выполнения запроса к модели таким образом, что входные данные могут параллельно вызывать различные части кода модели. Pipeline parallelism может быть эффективной техникой для:

- крупномасштабного обучения
- работы с кластерами с ограниченной пропускной способностью
- генерации ответов для больших моделей

### Low-Rank Adaptation (LoRA)

LoRA (Low-Rank Adaptation) — это эффективная техника дообучения больших языковых моделей, которая направлена на снижение вычислительных затрат и объема памяти при fine-tuning, сохраняя при этом высокое качество модели. LoRA добавляет небольшие низкоранговые матрицы к весам модели, которые обновляются во время дообучения, а основной корпус параметров модели остаётся неизменным. Этот подход существенно снижает необходимость обновления всех параметров модели.
