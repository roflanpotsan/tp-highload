...
- По данным OpenAI [[4](#список-источников)] размер модели GPT-3 был около 800 ГБ, размер тренировочных данных ~ 950 ГБ. Так как неизбежно будут выходить новые модели, то следует соответствующим образом рассчитать размер хранилища.

| Данные  | Объем |
| --- | --- |
| Обученная модель | > 800 ГБ |
| Данные для обучения | > 950 ГБ |
| Данные для дообучения | > 20 ГБ/мес |
...
- Для n = 2 получим (21,3 * (121 + 121 * 1,1) * 10 ^ 6) МБ + (800 ГБ + 950 ГБ + 20 ГБ * 12) * 2 ~ **5,42 ПБ**
...
### Схема Anycast балансировки
  - BGP Anycast
    - Объявление единого IP-адреса несколькими узлами
      - Несколько серверов или маршрутизаторов, расположенных в разных географических регионах, конфигурируются для использования одного и того же Anycast IP-адреса.
      - Маршрутизаторы в интернете принимают решение о том, какой из объявленных маршрутов является наиболее предпочтительным на основе различных критериев BGP, таких как наименьший AS-путь (автономная система), политика локального провайдера и другие метрики.
      - В результате трафик от клиента направляется к ближайшему (с точки зрения BGP) узлу, объявляющему Anycast IP-адрес.
      - Если один из узлов становится недоступным, маршрутизаторы автоматически перенаправляют трафик к следующим ближайшим узлам.
      - Трафик может распределяться между несколькими узлами, обеспечивая балансировку нагрузки и оптимальное использование ресурсов.
### Список источников
